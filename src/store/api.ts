...
export async function fetchGeminiResponse(
  ...
  if (isInOnboarding) {
    const lastAssistant = [...history].reverse().find(m => m.sender === 'assistant')?.content;
    const cleanLastAssistant = lastAssistant ? lastAssistant.replace(/\[FollowUps:\s*[^\]]+\]/gi, '').trim() : '';
    const lastAssistantCtx = cleanLastAssistant ? ` The user was just asked the question: "${cleanLastAssistant}".` : '';
    questionOverride = `\n\nCRITICAL INSTRUCTION: The user is currently in the onboarding flow and has sent a general query/question: "${prompt}".${lastAssistantCtx} You MUST directly, clearly, and concisely answer their query/question in 2-3 sentences. Do NOT output any standard welcome greeting, introduction, or campaign starting script (like "Hello! I am Dr. Dia..."). Focus entirely on answering their query.`;
  }
  ...
  try {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        trimmed = replyText.trim();
        // Clean any duplicate name/onboarding questions generated by the LLM
        trimmed = trimmed.replace(/(?:to get started|to begin|first|firstly|first off)?,?\s*(?:what should i call you|what is your name|what's your name|could you tell me your name|can i get your name|let's start with your name|may i know your name|could you start by sharing your name|please share your name to get started|what can i call you|how should i address you)\??/gi, '').trim();
        break; // Success! Break out of the retry loop.
      } catch (err) {
        if (attempts >= maxAttempts) {
          throw err;
...
export async function extractOnboardingEntities(
  ...
  if (!res.name && (!currentStep || currentStep === 'asked_name')) {
    ...
    if (nameMatch && nameMatch[1]) {
      ...
      const firstCommaPart = content.split(',')[0].trim();
      ...
      const isSentence = containsQuestionOrVerb || content.includes('?') || firstCommaPart.split(/\s+/).length > 2;

      if (!isSentence) {
        const firstWord = firstCommaPart.split(/\s+/)[0];
